...
var ClassicTheme = function (_React$PureComponent) {
  ...
  _createClass(ClassicTheme, [{
    ...
    value: function render12Hours() {
      ...
      var sliceLow = 0;
      var sliceHigh = _const_value.TIMES_12_MODE.length;
      if (props.withMinTime) sliceLow = _const_value.TIMES_12_MODE.findIndex(function (time) {
        return time === props.withMinTime;
      });
      if (props.withMinTime) sliceHigh = _const_value.TIMES_12_MODE.findIndex(function (time) {
        return time === props.withMinTime;
      });
      return [].concat(_toConsumableArray(_const_value.TIMES_12_MODE)).slice(sliceLow, sliceHigh).map(function (hourValue, index) {
    ...
    value: function render24Hours() {
      ...
      var sliceLow = 0;
      var sliceHigh = _const_value.TIMES_24_MODE.length;
      if (props.withMinTime) sliceLow = _const_value.TIMES_24_MODE.findIndex(function (time) {
        return time === props.withMinTime;
      });
      if (props.withMinTime) sliceHigh = _const_value.TIMES_24_MODE.findIndex(function (time) {
        return time === props.withMaxTime;
      });
